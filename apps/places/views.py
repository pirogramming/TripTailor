...
def get_recommendation_context(prompt, followup, user):
    """벡터 검색 기반 추천 컨텍스트 생성 - 임시로 기존 방식 사용"""
    # 임시로 기존 방식 사용 (벡터 검색 비활성화)
...
def get_more_recommendations_context(prompt, followup, user):
    """벡터 검색 기반 더 많은 추천 결과 - 임시로 기존 방식 사용"""
    # 임시로 기존 방식 사용 (벡터 검색 비활성화)
...
def place_detail(request, pk):
    ...
    # 사용자의 루트 목록 가져오기 (로그인한 경우에만)
    user_routes = []
    if request.user.is_authenticated:
        from apps.routes.models import Route
        user_routes = Route.objects.filter(creator=request.user).order_by('-created_at')
